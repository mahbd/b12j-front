   ...
   return (
      <div key={comment.id} id={`${commentCat}_comment_${comment.id}`}>
         ...
         <div className={"text-secondary"}>
            <small>
               By: {userActs.fullName(comment.by)} Date: {extractDate(comment.date)}
            </small>
         </div>

         {getCurrentUser() && (
            <div>
               <span onClick={() => replyFunction(comment.id, commentCat)} className={"text-primary clickable"}>
                  Reply{" "}
               </span>
               {getCurrentUser().id === comment.by && (
                  <span onClick={() => deleteFunction(comment.id)} className={"text-danger clickable"}>
                     Delete
                  </span>
               )}
            </div>
